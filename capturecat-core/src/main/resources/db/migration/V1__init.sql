...
(
    ...
    password           varchar(70),
    role               varchar(255) not null constraint users_role_check check ((role)::text = ANY((ARRAY ['ADMIN'::character varying, 'PREMIUM_USER'::character varying, 'USER'::character varying])::text[])),
...
(
    ...
    created_date         timestamp(6) not null,
    last_modified_date   timestamp(6) not null,
    image_cleanup_status varchar(255) not null constraint withdraw_log_image_cleanup_status_check check ((image_cleanup_status)::text = ANY((ARRAY ['PENDING'::character varying, 'DONE'::character varying, 'FAILED'::character varying])::text[])),
    reason               text,
